...
class VSM(object):
    def __init__(self, qry_path = None, rel_path = None, isTraining = True, doc_path = None):
        ...
        if qry_path == None: 
            qry_path = "../TDT2/Train/XinTrainQryTDT2/QUERY_WDID_NEW"
        if doc_path == None:
            doc_path = "../TDT2/SPLIT_DOC_WDID_NEW"
        if rel_path == None:
    ...
    def saveMdl(self, obj_qry, data_path):
        # save list to file.npy
        np.save(data_path + "/qry_IDs.npy", self.qry_IDs)
        np.save(data_path + "/doc_IDs.npy", self.doc_IDs)
        # save numpy array to file.npy
        ...
        # save objective (numpy array) to file.npy
